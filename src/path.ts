...
const op = (values: readonly number[], operator: string) => `${numbers(values)} ${operator}\n`

function path_commands(commands: readonly PathCommand[]): string {
  const parts: string[] = []
  ...
  for (const command of commands) {
    switch (command.kind) {
      case 'M':
        ...
        parts.push(op([command.x, command.y], 'm')); break
      case 'L': parts.push(op([command.x, command.y], 'l')); break
      case 'Q':
        parts.push(op([x + 2 / 3 * (command.x1 - x), y + 2 / 3 * (command.y1 - y),
          command.x + 2 / 3 * (command.x1 - command.x),
          command.y + 2 / 3 * (command.y1 - command.y), command.x, command.y], 'c'))
        break
      case 'C':
        parts.push(op([command.x1, command.y1, command.x2, command.y2, command.x, command.y], 'c'))
        break
...
function rect_path(rect: PixelRect, radius?: RectRadii): string {
  const { x, y, width, height } = rect
  if (!radius) return op([x, y, width, height], 're')
  ...
  return op([x + tl.x, y], 'm') + op([right - tr.x, y], 'l')
    + op([right - (1 - K) * tr.x, y, right, y + (1 - K) * tr.y, right, y + tr.y], 'c')
    + op([right, bottom - br.y], 'l')
    + op([right, bottom - (1 - K) * br.y, right - (1 - K) * br.x, bottom, right - br.x, bottom], 'c')
    + op([x + bl.x, bottom], 'l')
    + op([x + (1 - K) * bl.x, bottom, x, bottom - (1 - K) * bl.y, x, bottom - bl.y], 'c')
    + op([x, y + tl.y], 'l')
    + op([x, y + (1 - K) * tl.y, x + (1 - K) * tl.x, y, x + tl.x, y], 'c') + 'h\n'
}

function ellipse_path(center: Point, radius: Point): string {
  const { x, y } = center, rx = radius.x, ry = radius.y
  return op([x + rx, y], 'm')
    + op([x + rx, y + K * ry, x + K * rx, y + ry, x, y + ry], 'c')
    + op([x - K * rx, y + ry, x - rx, y + K * ry, x - rx, y], 'c')
    + op([x - rx, y - K * ry, x - K * rx, y - ry, x, y - ry], 'c')
    + op([x + K * rx, y - ry, x + rx, y - K * ry, x + rx, y], 'c') + 'h\n'
}
...
function square_caps(commands: readonly PathCommand[], width: number): string {
  let x = 0, y = 0, point = true, segment = false
  const parts: string[] = []
  const flush = () => {
    if (point && segment) parts.push(rect_path({ x: x - width / 2, y: y - width / 2, width, height: width }))
  }
  for (const command of commands) {
    if (command.kind === 'M') {
